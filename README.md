...
找到包含该星团的图像并下载。下载的波段是F814W、F475，储存位置是'.../m31/origion/HST/jbf*'。
...
⭐运行photometry.py代码：
    ...
    出现“请输入有效半径 (arcsec)”，输入星团的Reff；
    出现“配置信息确认:”，检查配置信息是否正确，储存位置是否正确，如确定无误，输入“y”。
    输入和保存目录为
      原始数据目录: '.../m31/origion/raw/jbf*'
      输出目录: '.../m31/JHONSON/J{APID}/data'
    这个代码会根据输入的星团坐标和有效半径剪裁HST图像，并自动生成PSF测光所需参数的.param文件，调用DOLPHOT进行测光，并提取DOLPHOT输出星表中有效源（不过曝、判定为恒星），并把这些源的信息转为jh2000格式，储存在'.../m31/JHONSON/J{APID}/{APID}.txt'中

⭐运行CMD.py代码：
    根据研究的星团，修改代码中“APID”，代码会自动绘制该星团的CMD图像，可以根据需要自行修改上下限。
    图像会被保存在.../m31/JHONSON/J{APID}/{APID}.png

⭐运行iso.py代码,会自动读取MIST的等龄线进行等龄线拟合和绘制。
